...
# Create schema on ALL ClickHouse pods (for load balancing)
echo ""
echo "🗄️  Creating database schema on all ClickHouse pods..."

# Get all ClickHouse pod names
PODS=$(kubectl get pods -n clickhouse -l clickhouse.altinity.com/app=chop -o jsonpath='{.items[*].metadata.name}')

# STEP 1: Drop existing tables on all pods
echo "🧹 STEP 1/2: Dropping existing tables on all pods..."
for pod in $PODS; do
  echo "  Dropping on $pod..."
  kubectl exec -n clickhouse $pod -- clickhouse-client --query "DROP TABLE IF EXISTS benchmark.cpu_local" 2>/dev/null
  kubectl exec -n clickhouse $pod -- clickhouse-client --query "DROP TABLE IF EXISTS benchmark.sensors_local" 2>/dev/null
done
echo "✅ All tables dropped!"

# STEP 2: Create tables on all pods
echo ""
echo "📝 STEP 2/2: Creating tables on all pods..."
SUCCESS=true
for pod in $PODS; do
  echo "  Creating on $pod..."
  cat 01-create-schema.sql | kubectl exec -i -n clickhouse $pod -- clickhouse-client --multiquery
  if [ $? -ne 0 ]; then
    echo "  ❌ Failed on $pod"
    SUCCESS=false
  else
